...
# def accuracy(cos_thetas, targets, topk=(1,0)):
...
#     val_acc = bingo_cnt.float() / float(128)
...
def config_dist_env_and_opt(opt):
    ...
    if opt.dist_mode == 'cpu':
        ...
        print("debug for horovod mode init \n")
...
def load_model(model, load_parameters_from, strict_load=False, map_location='cpu', only_backbone=False ):
...
def config_model_optimizer_hvd_and_apex(model, optimizer, opt):
    ...
    # print("debug in horovod: ", optimizer)
...
def validate(val_loader, model, criterion, opt, epoch='N/A', add_margin=False):
    losses = AverageMeter('Loss ', ':6.4g')
    top1 = AverageMeter('Acc@1 ', ':6.2f')
    top5 = AverageMeter('Acc@5 ', ':6.2f')
...
def train_all_epochs(opt, model, optimizer, train_sampler, train_loader, criterion, val_loader, num_train_samples=None,
                     no_acc_eval=False, save_all_ranks=False, training_status_info=None, save_params=True, add_margin=False):
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        if train_sampler is not None and opt.dataset not in ['AffectNet', 'AffectNet8']:
        ...
        if val_loader is not None and not opt.no_acc_eval:
        ...
        logging.info(
            ...
            'losses={:4g}, best_acc1={:4g}, best_acc1_at_epoch={}, best_acc5={}, best_acc5_at_epoch={}'.format(
...
def main(opt, argv):
    ...
    # logging.info("data_loader_info.....", str(data_loader_info))
    ...
    print("train_sampler is: ....", type(train_sampler))
    ...
    if opt.load_parameters_from:
        model = load_model(model, opt.load_parameters_from, opt.strict_load, map_location='cpu', only_backbone=opt.only_backbone)

        if opt.freeze_backbone:
            for param in model.parameters():
                param.requires_grad = False
            for param_linear in model.fc_linear.parameters():
                param_linear.requires_grad = True

    if opt.fp16:
        model = model.half()
        print("log in model half")

    # set device
    if opt.gpu is not None:
        print("debug in gpu set: ", opt.gpu)
        torch.cuda.set_device(opt.gpu)
        model.cuda(opt.gpu)
    ...
    elif (hasattr(opt, 'suploss') and opt.suploss):
    ...
    if not opt.evaluate_only:
        training_status_info = train_all_epochs(opt, model, optimizer, train_sampler, train_loader, criterion, val_loader,
                         ...
                         no_acc_eval=opt.no_acc_eval, save_all_ranks=False,
                         training_status_info=training_status_info)
    else:
